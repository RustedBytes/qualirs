...
| Q0004 | Cyclic Crate Dependency | Exploratory coupling hint for more than five explicit crate-relative import roots. Imports alone cannot establish a cycle; external imports and imports from a module's own children do not establish cycles. | Imports from six distinct `crate::root::Item` paths | Inspect the actual dependency graph before refactoring |
...
| Q0010 | Test-only Dependency in Production | Source imports a dependency declared only for development. Normal, optional, renamed, workspace-inherited, and target dependency entries are accounted for. Known crate entry points are medium confidence; unresolved module or documentation contexts are low. | `[dev-dependencies] helper = "1"` with `use helper::run;` in the production entry point | Declare production requirements under `[dependencies]`; retain dev-only entries for tests and documentation |
...
| Q0051 | Repeated Regex Construction | Resolved regex constructors compile a literal pattern at runtime; dynamic patterns and unresolved APIs are exploratory. | `for s in lines { regex::Regex::new("[a-z]+").unwrap().is_match(s); }` | Cache a reused fixed pattern; dynamic patterns need query-scoped or keyed caching |
...
| Q0055 | Vec Contains in Loop | A known Vec from outside a loop is searched inside it; freshly created vectors are exploratory because reuse is unproven. | `fn check(v: Vec<u32>) { for x in ids { v.contains(&x); } }` | Consider a companion set when lookup cost dominates; preserve ordering and avoid building a set for one search |
...
| Q0061 | Repeated Expensive Construction in Loop | Resolved parser constructors receive literal input in an executed loop body. Iterator setup and deferred bodies are excluded; unresolved names and owned path construction are low confidence. | `for item in items { let url = url::Url::parse("https://example.com"); ... }` | Reuse a parsed URL when ownership permits; retain separate owned values when needed |
...
| Code | Item | What it catches | Bad example | Good example |
|---|---|---|---|---|
| Q0067 | Excessive Unwrap | Frequent unwraps of established Results; unresolved calls and invariants are exploratory. Proven equal-size byte-slice conversions and NUL-free CString literals are excluded. | Repeated `std::fs::read(path).unwrap()` | Handle fallible operations; retain proven invariants. Nested execution bodies do not inflate the parent count |
| Q0068 | Unused Result Ignored | A Result is explicitly discarded with `let _ =`; unresolved calls, documented best-effort code (including an explained cleanup loop), and destructor file cleanup are exploratory. Loop explanations do not cover later statements or deferred bodies. | `let _ = std::fs::write(path, bytes);` | Handle errors or document intentional discard; cleanup findings concern observability, not necessarily application failure |
...
| Q0073 | Manual Default Constructor | A non-const constructor initializes proven default fields without an existing local Default implementation. | `impl Bag { fn new() -> Self { Self { items: Vec::new() } } }` | Consider deriving Default; existing delegation and Default implementations are excluded to avoid redundant or recursive advice |
| Q0074 | Manual Option/Result Mapping | Simple runtime matches use resolved standard variants and irrefutable bindings. Const contexts, guards, refutable patterns, and arms containing await, early exits, or opaque macros are excluded. Transforming both Result branches is low confidence because closure captures may conflict. | `match opt { Some(x) => Some(x + 1), None => None }` | `opt.map(\|x\| x + 1)`; retain matches needed for async work or error propagation |
| Q0075 | Manual Find/Any Loop | A loop contains only a conditional boolean return, with no else branch, extra branch work, or predicate await/early-exit/opaque macro. | `for x in xs { if pred(x) { return true; } }` followed by `false` | Consider `xs.into_iter().any(pred)` while preserving short-circuit behavior |
...
| Q0077 | Derivable Impl | Bounded checks establish equivalent fieldwise Default/Clone or empty Eq implementations on nongeneric local structs. Eq additionally requires proven field trait bounds. | `struct Key(u64); impl Eq for Key {}` with a valid PartialEq implementation | Derive Eq only when every field supports it; float wrappers and unresolved custom fields are excluded |
...
| Code | Item | What it catches | Bad example | Good example |
|---|---|---|---|---|
| Q0087 | Unsafe Without Comment | Unsafe code lacks a structurally associated safety explanation. Explicit markers, bounded natural rationales, and attribute comments are recognized. A note naming one unsafe call and a validity condition can cover that call inside a synchronous closure in the same statement. | `unsafe { ptr.read() }` with no explanation | Explain pointer validity and caller obligations; generic construction notes, unrelated statements, nested functions, and async bodies do not inherit closure-call notes |
...
| Q0091 | FFI Without Wrapper | No call from a local safe function or method can be resolved for a foreign declaration. Calls establish wrapper evidence, not similar names. Missing coverage is low confidence because wrappers may live elsewhere. | `unsafe extern "C" { fn raw(); }` with no resolved safe caller | A safe function or method calls the foreign function and validates its inputs |
...
| Q0094 | Unsafe Impl Missing Safety Docs | A resolved Send/Sync implementation lacks a safety explanation in documentation or ordinary comments. Bounded causal explanations such as "safe here since ..." and "..., so this is safe" count as documentation without a literal SAFETY label. This does not establish soundness. | `unsafe impl Send for Buffer {}` with no comment | Explain the invariants; adjacent marker impls for the same type may share an explanation |
