...
| Q0094 | Unsafe Impl Missing Safety Docs | A resolved Send/Sync implementation lacks a safety explanation in documentation or ordinary comments. A literal SAFETY label is not required when a bounded rationale is recognized. | `unsafe impl Send for Buffer {}` with no comment | Explain the invariants; adjacent marker impls for the same type may share an explanation |
