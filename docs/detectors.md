...
| Code | Item | What it catches | Bad example | Good example |
|---|---|---|---|---|
| Q0087 | Unsafe Without Comment | Unsafe code lacks a structurally associated safety explanation. Explicit safety markers and plain explanations of invariants, thread guarantees, and reference ownership are recognized, including comments trailing attributes. | `unsafe { ptr.read() }` with no explanation | Explain pointer validity and caller obligations; string literals and unrelated statement/function comments do not count |
