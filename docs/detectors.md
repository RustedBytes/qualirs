...
| Code | Item | What it catches | Bad example | Good example |
|---|---|---|---|---|
| Q0087 | Unsafe Without Comment | Unsafe code lacks a structurally associated safety explanation. Multiline comments, attributes, match arms, explicit operation groups, and function preconditions are recognized. | `unsafe { ptr.read() }` with no explanation | Explain pointer validity and caller obligations; string literals and unrelated function comments do not count |
...
| Q0094 | Unsafe Impl Missing Safety Docs | A resolved Send/Sync implementation lacks a safety explanation in documentation or ordinary comments. | `unsafe impl Send for Buffer {}` with no comment | Explain the invariants; adjacent marker impls for the same type may share an explanation |
